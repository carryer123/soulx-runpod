...
if __name__ == "__main__":
    ...
    _fa = _FastAPI()
    @_fa.get("/manifest.json", include_in_schema=False)
    def _manifest(): return _JSONResponse({"name":"혜린 라이브 아바타","short_name":"혜린","start_url":"/","display":"standalone","icons":[]})

    from fastapi import HTTPException, UploadFile, File, Form
    from fastapi.responses import FileResponse
    import os as _os, glob as _glob, shutil as _shutil, subprocess as _subprocess, tempfile as _tempfile, uuid as _uuid
    @_fa.post("/api/v1/inference")
    async def fewoov_inference(
        portrait: UploadFile = File(...),
        audio: UploadFile = File(...),
        model_type: str = Form("lite"),
        seed: int = Form(9999),
        use_face_crop: bool = Form(False),
        lora_choice: str = Form("OFF (base Lite)"),
    ):
        """portrait+audio multipart → 합쳐진 mp4 binary 반환."""
        try:
            # 임시 파일 저장
            tmp = _tempfile.mkdtemp(prefix="fewoov_")
            portrait_path = _os.path.join(tmp, portrait.filename or "portrait.jpg")
            audio_path = _os.path.join(tmp, audio.filename or "audio.wav")
            with open(portrait_path, "wb") as f: _shutil.copyfileobj(portrait.file, f)
            with open(audio_path, "wb") as f: _shutil.copyfileobj(audio.file, f)

            # streaming inference 끝까지 소비 (segments mp4 path 모음)
            segments = []
            for seg_path in run_inference_streaming(
            ...
            out_path = _os.path.join(_OUT_DIR, f"talking_{out_id}.mp4")
            list_path = _os.path.join(tmp, "concat.txt")
            with open(list_path, "w") as f:
                for s in segments: f.write(f"file '{s}'\n")
            # video 만 concat (segments 경계 audio glitch 회피) + 원본 wav 1회 mux
            _subprocess.run(
            ...
            # cleanup tmp
            try: _shutil.rmtree(tmp)
            except: pass
            return FileResponse(out_path, media_type="video/mp4", filename=f"talking_{out_id}.mp4")
        except HTTPException:
            raise
        except Exception as e:
            import traceback; traceback.print_exc()
            raise HTTPException(500, f"inference failed: {e}")
    _fa = gr.mount_gradio_app(_fa, app, path="/", allowed_paths=["/tmp"])

    # ───── Fewoov 라이브 채팅용 native API (gradio session 우회) ─────
    from fastapi import HTTPException, UploadFile, File, Form
    from fastapi.responses import FileResponse
    import os as _os, glob as _glob, shutil as _shutil, subprocess as _subprocess, tempfile as _tempfile, uuid as _uuid

    _OUT_DIR = "/home/moon1/SoulX-FlashHead/fewoov_out"
    _os.makedirs(_OUT_DIR, exist_ok=True)
    _uvicorn.run(_fa, host="0.0.0.0", port=7864)